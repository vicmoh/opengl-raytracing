  ...
  for (i = 0; i < SIZE; i++) {
    for (j = 0; j < SIZE; j++) {
      ...
      if (dis == 0 || dis > 0) {
        ...
        if (DEBUG) printf("nx: %f, ny: %f, nz: %f\n", n.x, n.y, n.z);
        ...
        if (DEBUG) print("clacViewAngle -> ", pointToString(v));
